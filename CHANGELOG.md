...
- The EQ canvas/readout/footer backgrounds now use the same native `NSVisualEffectView` sidebar material as the preset sidebar (`SidebarMaterialBackground`), replacing a flat `windowBackgroundColor` fill that matched in light mode but visibly diverged from the sidebar's vibrant material in dark mode. It also correctly dims when the window loses focus, matching the native sidebar list instead of always rendering as if key. `DreamTheme.bgWindow`/`bgToolbar`/`bgCanvas`/`bgTitlebar` are removed as unused. The sidebar's favorite-star accent color changed from a hardcoded yellow to the system accent color (`NSColor.controlAccentColor`).
