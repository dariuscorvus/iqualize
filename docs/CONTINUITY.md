...
## 2026-07-12: Ported to current main as fix/airpods-handoff-v2

`fix/airpods-handoff-yield` was based on a commit from 2026-05-23, well before
per-device EQ memory (#79), the CLI, the deletable-presets catalog, and the
#87 Discord/late-launched-app fix all landed on `main`. Rather than rebase a
large, partially-conflicting diff, the process-split fix (the one piece with
a rigorous causal proof — see above) was ported fresh onto current `main` as
`fix/airpods-handoff-v2`.

**Deliberately not ported in this pass**: the `preferredOutputUID`/idle-on-
fallback state machine and the silence-yield pause/resume monitor. Both were
built iteratively on the old branch while other variables were still
changing, and neither has a causal proof behind it the way the split does —
the PR's own test plan only validates the split's core case (mid-playback
Mac→iPhone). This version does plain default-output-following on every route
change, nothing more. Plan: validate this minimal version against the full
matrix (mid-playback handoff both directions, auto-return, sleep/wake,
repeated EQ on/off) before deciding whether either heuristic needs to come
back. If the split alone covers auto-return too, they can stay cut.

**Preserved as-is**: the #87 new-process tap-coverage polling
(`knownProcessObjectIDs`/`scheduleProcessListPoll`/`pollProcessList` in
`AudioEngine.swift`) needed no changes — `restartTap()` already goes through
`stop()`/`start()`, which now respawns the capture helper (and therefore its
tap) under the hood, so a newly-launched app still gets picked up the same
way it did pre-split.
